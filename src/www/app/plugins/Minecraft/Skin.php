...
class Skin {
  ...
  public function __construct($username = null, $size = 48) {
    $this->username = $username;
    $this->size = $size;

    error_log("creating avatar for ". $this->username);
  }

  public function getFace() {
    $this->image = imagecreatetruecolor($this->size, $this->size);
    
    $image = imagecreatefromstring($this->getFaceData());

    imagecopyresized($this->image, $image, 0, 0, 8, 8, $this->size, $this->size, 8, 8);         // Face
    imagecolortransparent($image, imagecolorat($image, 63, 0));                       // Black Hat Issue
    imagecopyresized($this->image, $image, 0, 0, 8 + 32, 8, $this->size, $this->size, 8, 8);    // Accessories
  ...
  private function getFaceData() {
    // Just in case, drop the default Steve skin in the output.
    ...
    if (! is_null($this->username)) {
      $fetch = "http://skins.minecraft.net/MinecraftSkins/".$this->username.".png";

      error_log("sending request to ". $fetch);
